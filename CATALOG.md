...
| [`modernize-kit`](plugins/modernize-kit/) | Accelerator | `framework-modernizer` skill (Express 4→5) — catalog of breaking-change patterns + SAFE/AUTOFIX/MANUAL classifier rubric + eval fixture + phased-plan template. Roadmap: Next 14→15, React 17→18, Spring Boot 2→3, Java 8→17. | …have a planned framework migration. **Episodic, opt-in** — pin during the migration, remove afterwards. |
...
`modernize-kit` shipped in v6.0.0 with a single skill (`framework-modernizer`, Express 4→5). It is opt-in — consumers add it when they have a planned framework migration and remove it once the migration completes; accelerators are **episodic**, not compounding. Leaving an unused accelerator pinned isn't dangerous, but it muddies the "what is this repo asking the agent to do today" signal. Future accelerators (Next 14→15, React 17→18, Spring Boot 2→3, Java 8→17) will ship as additional skills inside the same kit, so consumers pin once and select the relevant skill at invocation.
...
|---|---|
| `plugins/modernize-kit#v6.0.0` | You have a planned framework migration. Today: Express 4→5. Roadmap: Next 14→15, React 17→18, Spring Boot 2→3, Java 8→17. **Remove the pin once the migration completes** — accelerators are episodic, not compounding. |
